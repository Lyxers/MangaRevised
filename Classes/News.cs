...
namespace MangaDownloaderRevised.Classes
{
    public class News : System.Windows.Controls.ListBox
    {
        public void initNews(string configPath)
        {
            ...
            worker.RunWorkerAsync(configPath);
    
        }

        private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            string[] newsItems = System.IO.File.ReadAllLines((string)e.Argument);
        ...
        public static string[] newsMangaice(string newsItem, Regex matchmaker)
        {
            try
            {
        ...
        public void addToListBox(string[] newsPackage)
        {
            if (newsPackage != null)
            {
                Dispatcher.BeginInvoke((Action)(() => {this.Items.Add(newsPackage[1] + "\nby " + newsPackage[0] + "\nat " + newsPackage[2]); }));
            }
